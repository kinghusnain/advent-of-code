# Lint as: python3
"""Advent of Code, Day 9 -- Signal Boost (intcode v4).

>>> cpu = IntcodeComputer()
>>> p = '109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99'
>>> m = cpu.load_program(p)
>>> m = cpu.run()
>>> p == ','.join([str(i) for i in cpu.output])
True
>>> cpu = IntcodeComputer()
>>> m = cpu.load_program('1102,34915192,34915192,7,4,7,99,0')
>>> m = cpu.run()
>>> len(str(cpu.output[-1])) == 16
True
>>> cpu = IntcodeComputer()
>>> m = cpu.load_program('104,1125899906842624,99')
>>> m = cpu.run()
>>> cpu.output[-1]
1125899906842624
"""
